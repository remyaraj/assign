...
class FutureSymbol
	...
	def get_symbol	
		k = @time.month
		k = k+1 if expired?
		@future_symbol[1] = case_value(k,@es_case,"ES")
		@future_symbol[2] = case_value(k,@es_case,"YM")
		@future_symbol[3] = case_value(k,@gc_case,"GC")	
		return @future_symbol[rand((@future_symbol.length)-1)+1]
	end
	def get_allsymbols
		get_symbol
